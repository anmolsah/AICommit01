...
function activate(context) {
  ...
  setTimeout(() => {
    vscode.window.showInformationMessage(
      "✨ Commit Genius is ready! Look for the sparkle button in Source Control."
    );
  }, 200);
...
async function generateCommitMessage(context) {
  ...
  try {
    if (shouldStage) {
      ...
      vscode.window.showInformationMessage(
        `📁 Staged ${filePaths.length} file(s)`
      );
    } else {
      const diffPromises = changesToAnalyze.map(async (change) => {
        try {
          ...
          console.warn(
            `Could not get staged diff for ${change.uri.fsPath}:`,
            error
          );
  ...
  if (!diffContent.trim()) {
    throw new Error(
      "Could not analyze changes. Please ensure you have valid Git changes."
    );
  }
